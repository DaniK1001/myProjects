...
public class Driver {
    ...
    /**
     * This method displays words that start with a specified letter.
     * It prompts the user to enter a letter, then searches for words in the vocabulary control center
     * that start with the specified letter. It prints the list of words in alphabetical order.
     */
    ...
    /**
     * This method sorts the list of words that start with a specified letter in alphabetical order.
     */
    private static void sortSameLetter() {
        for (int i = 0; i < sameLetter.size() - 1; i++) {
            for (int j = 0; j < sameLetter.size() - i - 1; j++) {
                if (sameLetter.get(j).compareTo(sameLetter.get(j + 1)) > 0) {
                    String temp = sameLetter.get(j);
                    sameLetter.set(j, sameLetter.get(j + 1));
                    sameLetter.set(j + 1, temp);
                }
            }
        }
    }
